...
ok('H2 uses official keyboard Space before player selection', 'PostStartMenuScene = "menu_post_start_main"' in plugin and 'PressQaKeyboardSpace(320)' in plugin and 'Rewired.Keyboard Space' in plugin)
ok('H2 patches Rewired keyboard controller', 'PatchQaKeyboardPostfix' in plugin and 'KeyboardKeyCode.Space' in plugin and 'KeyCode.Space' in plugin)
