...
ok('version 0.9.10', 'PluginVersion = "0.9.10"' in plugin and '<Version>0.9.10</Version>' in (root/'UnspottableExpanded.csproj').read_text())
...
ok('H2 uses official keyboard Space before player selection', 'PostStartMenuScene = "menu_post_start_main"' in plugin and 'PressQaOfficialKeyboardSpace(320)' in plugin and 'Rewired.Keyboard Space' in plugin)
ok('H2 patches Rewired keyboard controller', 'PatchQaKeyboardPostfix' in plugin and 'KeyboardKeyCode.Space' in plugin and 'KeyCode.Space' in plugin and 'InjectQaMappedSpaceForPlayer' in plugin and 'controllers.hasKeyboard' in plugin and 'ControllerType.Keyboard' in plugin)
ok('H2 no direct Unity UI submit fallback', 'TrySubmitQaLocalMenuUi' not in plugin and 'InvokeQaUiSubmit' not in plugin)
