...
H2.6 uses a visible rendered game process and preserves the game's normal scene lifecycle. Instead of assuming Rewired Player 0 is the keyboard player, pre-game selection and P1 join now emulate the official keyboard Space control using the game's real Rewired keyboard ownership/maps. H2 scans the System Player and game Players that actually own the keyboard, finds enabled actions mapped to Space, injects those exact actions for those exact owners, and also keeps a direct `Rewired.Keyboard` Space hook as a fallback. After P1 exists, the existing normal lifecycle continues.
